...
export function mockServiceDrive(options: { onUpload?: (count: number) => Promise<void>; onMetadata?: (count: number) => Promise<void>; failUpload?: number } = {}) {
  ...
  const files = new Map<string, { body: string; mimeType: string; appProperties?: Record<string, string>; parents?: string[] }>();
  const mock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.pathname === "/drive/v3/files/generateIds") return jsonResponse({ ids: Array.from({ length: Number(url.searchParams.get("count")) }, () => `generated_${prefix}_${++generatedCount}`) });
    if (url.pathname === "/upload/drive/v3/files" && init?.method === "POST") {
      ...
      const id = meta.id ?? `service_file_${prefix}_${uploadNumber}`;
      if (files.has(id)) return jsonResponse({ error: "already_exists" }, 409);
      files.set(id, { body: content, mimeType: meta.mimeType, appProperties: meta.appProperties, parents: meta.parents });
      return jsonResponse({ id, name: meta.name, modifiedTime: "2026-09-10T00:00:00Z" });
    }
    if (url.pathname === "/drive/v3/files") {
      if (init?.method === "POST") {
        const meta = JSON.parse(String(init.body));
        const id = meta.id ?? "folder_auto";
        if (files.has(id)) return jsonResponse({ error: "already_exists" }, 409);
        files.set(id, { body: "", mimeType: meta.mimeType, parents: meta.parents, appProperties: meta.appProperties });
        return jsonResponse({ id, name: meta.name });
      }
    ...
    if (url.pathname.startsWith("/drive/v3/files/")) {
      ...
      return jsonResponse({ id, driveId: "drive_team", parents: file?.parents ?? ["folder_auto"], appProperties: file?.appProperties, size: String(new TextEncoder().encode(file?.body ?? "").byteLength), trashed: false, mimeType: id === "folder_auto" || id === "folder_root" ? "application/vnd.google-apps.folder" : file?.mimeType ?? "text/html",
